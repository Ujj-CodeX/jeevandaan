  ...
  return text.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())
    ...
    try {
        const response = await api.get(
            `http://43.204.39.169/api/chat/${this.requestId}/history/`,
            { params: { _t: Date.now() } }
        )
...
async submitDonorRating() {
    ...
    try {
        await api.post(
            `http://43.204.39.169/api/requests/donor/${this.requestId}/rate/`,
    ...
    try {
        ...
        if (this.userType === 'partner') {
            const response = await api.get(
                'http://43.204.39.169/api/requests/donor-requests/'
            )
            ...
            const response = await api.get(
                `http://43.204.39.169/api/requests/donor/${this.requestId}/detail/`
            )
    ...
    async sendMessage(message) {
      ...
      try {
        await api.post(`http://43.204.39.169/api/chat/${this.requestId}/send/`, { message })
    ...
    try {
        const response = await api.post(
            `http://43.204.39.169/api/requests/donor/${this.requestId}/cancel/`,
            {
...
async fetchDonorInfo() {
    ...
    try {
        const response = await api.get('http://43.204.39.169/api/users/profile/')
